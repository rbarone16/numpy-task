...
def prodotto_scalare(v1: list, v2: list) -> float:
    a = v1
    b = v2
    prodotto = np.dot(v1,v2)
    #np.dot accetta anche liste e non solo array
    return (prodotto)
    pass

def rango_matrice(m: list) -> int:
    M = m
    valore = int(np.linalg.matrix_rank(M))
    return(valore)
    pass
...
def operazioni_elemento_per_elemento(v1: list) -> tuple:
    ...
    v1
    a1 = float(np.sin(v1[0]))
    a2 = float(np.cos(v1[1]))
    a3 = float(np.arcsin(v1[2]))
    a4 = float(np.arccos(v1[3]))
    return(a1,a2,a3,a4)
    pass
